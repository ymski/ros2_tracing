...
class TestIntraPubSub(TraceTestCase):
        ...
        super().__init__(
            *args,
            ...
            events_ros=[
                ...
                tp.ring_buffer_enqueue,
                tp.ring_buffer_dequeue,
                tp.callback_start,
                tp.callback_end,
            ],
    ...
    def test_all(self):
        ...
        # init phase
        rclcpp_subscription_callback_added_events = self.get_events_with_name(tp.rclcpp_subscription_callback_added)
        ...
        construct_ring_buffer_events = self.get_events_with_name(tp.construct_ring_buffer)

        # runtime phase
        rclcpp_intra_publish_events = self.get_events_with_name(tp.rclcpp_intra_publish)
        ring_buffer_enqueue_events = self.get_events_with_name(tp.ring_buffer_enqueue)
        ring_buffer_dequeue_events = self.get_events_with_name(tp.ring_buffer_dequeue)
        ...
        print(f'dict: {buffer_to_callback}')
        ...
        for i, intra_publish_event in enumerate(rclcpp_intra_publish_events):
            ...
            target_enqueue_event = self.get_corresponding_event(self.get_field(intra_publish_event, '_timestamp'), enqueue_event_cand)
            ...
            filtered_dequeue_events = self.get_filtered_event({'buffer': target_buffer, 'index': target_index}, ring_buffer_dequeue_events)
            target_dequeue_event = self.get_corresponding_event(self.get_field(target_enqueue_event, '_timestamp'), filtered_dequeue_events)
            ...
            target_callback_start = self.get_corresponding_event(self.get_field(target_dequeue_event, '_timestamp'), filterd_callback_start)
            ...
            target_callback_end = self.get_corresponding_event(self.get_field(target_callback_start, '_timestamp'), callback_end_cand)

            print(f'\n\n~~~ publish: {i} ~~~')
            # print(f'filter dequeue events: \n key = None: {len(ring_buffer_dequeue_events)}  ->  key = buffer: {len(dequeue_events_b)}  ->  key = index: {len(dequeue_events_bi)}')
            # print(f'filter callback_start events: \n key = None: {len(callback_start_events)}  ->  key = callback_object: {len(callback_start_cand)}') # 重複してデータを使用していないかの判定
            # print(f'filter callback_end: \n key = None: {len(callback_end_events)}  ->  key = callback_object: {len(callback_end_cand)}') # 重複してデータを使用していないかの判定

            print(f'target intra publish event: \n {intra_publish_event}')
            print(f'corresponding enqueue event: \n {target_enqueue_event}')
            print(f'corresponding dequeue event:\n  {target_dequeue_event}')
            print(f'corresponding callback_start event: \n {target_callback_start}')
            print(f'corresponding callback_end event: \n {target_callback_end}')
